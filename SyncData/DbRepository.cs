    ...
    public class DbRepository
    {
        ...
        public int InsertCarinfo(ILog log, List<bx_carinfo> list)
        {
            ...
            if (list != null && list.Any())
            {
                ...
                foreach (var item in list)
                {
                    sql.Append($" ('{item.id}','{ReplaceSome(item.license_no)}','{ReplaceSome(item.engine_no)}','{ReplaceSome(item.vin_no)}','{ReplaceSome(item.mold_name)}','{item.register_date}','{item.owner_idno}','{ReplaceSome(item.license_owner)}','{item.owner_idno_type}','{item.license_type}','{item.car_type}','{item.car_used_type}','{item.seat_count}','{item.exhaust_scale}','{item.car_equ_quality}','{item.car_ton_count}','{item.purchase_price}','{item.fuel_type}','{item.proof_type}','{item.license_color}','{item.clause_type}','{item.run_region}','{item.create_time}','{item.update_time}','{item.transfer_date}','{ReplaceSome(item.auto_model_code)}','{item.VehicleYear}','{item.drivelicense_caryype_value}','{item.drivelicense_cartype_name}','{item.RenewalCarType}','{item.OwnerSex}','{item.OwnerBirthday}','{item.analogy_price}','{item.pa_auto_model_code}','{ReplaceSome(item.BrandName)}','{ReplaceSome(item.VehiclesName)}'),");
                }
            }
            if (sql.Length == 0)
            {
